	...
	drawGraph: function(obj) {
		...
		if ((gascVal != null) && (elecVal != null) && (sfcVal != null && sfcVal != 0)) {
			var str =
				...
				data.Gf[0].c[0].v +
				'</h5><h4 class="piechart-alignment-Elc piechart-amt">$' + data.Gf[
					0].c[
					1].v.toFixed(2) + '</h4></td>' +
				'<td class="text-center rmbrd bluColor"><div class="icon-gas-cb"></div><h5>' +
				data.Gf[1].c[0].v +
				'</h5><h4 class="piechart-alignment-Gas piechart-amt">$' + data.Gf[
					1].c[
					1].v.toFixed(2) + '</h4></td>' +
				'</tr>' +
				'<tr>' +
			...
			var str =
				...
				data.Gf[0].c[0].v + '</h5><h4 class="PADDTB8PX">$' + data.Gf[0].c[
					1].v.toFixed(
					2) + '</h4></td>' +
				'</tr>' +
				'</table>';
			...
			var str =
				...
				data.Gf[1].c[0].v + '</h5><h4 class="PADDTB8PX">$' + data.Gf[1].c[
					1].v.toFixed(
					2) + '</h4></td>' +
				'</tr>' +
				'</table>';
			...
			var str =
				...
				data.Gf[2].c[0].v + '</h5><h4 class="PADDTB8PX">$' + data.Gf[2].c[
					1].v.toFixed(
					2) + '</h4></td>' +
				'</tr>' +
				'</table>';
...
var BalanceData = Backbone.Collection.extend({
	url: 'http://localhost:3000/balanceData'
});
...
var CurrentBillData = Backbone.Model.extend({
	url: 'http://localhost:3000/currentBillData',
	defaults: {
		currentBillData: "",
		status: false,
		accountIndex: 0,
...
var billCmpModel = Backbone.Model.extend({
	...
	url: function() {
		...
		return 'http://localhost:3000/billComparison' + selectedSiteID;
	},
	defaults: {
		currentBillData: "",
		accountIndex: 0
...
var invoiceLineItemsModel = Backbone.Model.extend({
	...
	url: function() {
		...
		return 'http://localhost:3000/invoiceLineItems';
	}
});
...
var PaymentPlanModel = Backbone.Collection.extend({
	...
	url: function() {
		return 'http://localhost:3000/paymentPlanData';
	},
});

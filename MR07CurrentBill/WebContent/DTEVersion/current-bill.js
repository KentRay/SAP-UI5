...
var CurrentBillPageMainView = Backbone.View.extend({
    ...
    viewAllAccounts: function(ev) {
        $('#eBillEnroll').css('display', 'none');
        $('#eBillLearn').css('display', 'inline-block');
        this.options.model.set({
            status: false
        });
        new CurrentBillPageMainView({
    ...
    drawGraph: function(obj) {
        ...
        $('#EnrollNotification').css('display', 'none');
        currentEnroll.fetch({
            success: function(model, data) {
                var elgAccs = 0;
                if (data && data.accounts) {
                    //console.log(data.accounts[obj.accountIndex].eligiblePrograms);

                    var eligible = data.accounts[obj.accountIndex].eligiblePrograms;
                    for (var j = eligible.length - 1; j >= 0; j--) {
                        var isAPP = eligible[j].name.indexOf("EBL");
                        if (isAPP != -1)
                            elgAccs++;
                    }
                    if (elgAccs > 0)
                        $('#EnrollNotification').css('display', 'block');

                }
            },
            error: function(data) {
                console.log("in fail state", data);
            }
        });

        $('#EnrollNotification').css('display', 'none');
        var temp1 = getCookie('ACN');
        var accVal = $('.mcurrentbill-accno:first-child+section').text().replace(/\s/g, '');
        //this.accVal = accVal;
        console.log(accVal);
        if (temp1 === accVal) {
            //console.log("yes");
            //createExpirableCookie('ACN1',accVal, webSecurityTokenExpiry(), "/");
            $('#EnrollNotification').css('display', 'block');
            $('#eBillEnroll').css('display', 'block'); //display enroll
        } else {
            console.log("nooo"); //dont
        }
        //createExpirableCookie('ACN',obj.currentBillData.currentBillData.accounts[obj.accountIndex].accountNumber, webSecurityTokenExpiry(), "/");
        ...
        var chart = new google.visualization.PieChart(this.$('#piechart').get(
            0));
        ...
        if ((gascVal != null) && (elecVal != null) && (sfcVal != null && sfcVal != 0)) {
            var str =
                ...
                data.Lf[0].c[0].v +
                '</h5><h4 class="piechart-alignment-Elc piechart-amt">$' + data.Lf[
                    0].c[
                    1].v.toFixed(2) + '</h4></td>' +
                '<td class="text-center rmbrd bluColor"><div class="icon-gas-cb"></div><h5>' +
                data.Lf[1].c[0].v +
                '</h5><h4 class="piechart-alignment-Gas piechart-amt">$' + data.Lf[
                    1].c[
                    1].v.toFixed(2) + '</h4></td>' +
                '</tr>' +
                '<tr>' +
            ...
            var str =
                ...
                data.Lf[0].c[0].v + '</h5><h4 class="PADDTB8PX">$' + data.Lf[0].c[
                    1].v.toFixed(
                    2) + '</h4></td>' +
                '</tr>' +
                '</table>';
            ...
            var str =
                ...
                data.Lf[1].c[0].v + '</h5><h4 class="PADDTB8PX">$' + data.Lf[1].c[
                    1].v.toFixed(
                    2) + '</h4></td>' +
                '</tr>' +
                '</table>';
            ...
            var str =
                ...
                data.Lf[2].c[0].v + '</h5><h4 class="PADDTB8PX">$' + data.Lf[2].c[
                    1].v.toFixed(
                    2) + '</h4></td>' +
                '</tr>' +
                '</table>';
...
var BillCompare = Backbone.View.extend({
    ...
    render: function() {
        ...
        billCompareModel.fetch({
            ...
            success: function(model, response) {
                var data = {
                    "billCompareData": response.billComparison,
...
var invoiceLineItemsView = Backbone.View.extend({
    ...
    render: function() {
        ...
        invLineItemsModel.fetch({
            ...
            success: function(model, response) {
                var data = {
                    "invLineItmsData": response.invoiceLineItems,
...
var BalanceData = Backbone.Collection.extend({
    url: '/api/customer/balanceData'
});

var CurrentBillData = Backbone.Model.extend({
    url: '/api/currentBillData',
    defaults: {
        currentBillData: "",
        status: false,
        accountIndex: 0,
...
var billCmpModel = Backbone.Model.extend({
    ...
    url: function() {
        /*console.log("Bill Comp Model Fields Start");
        console.log("selectedSiteID", selectedSiteID);
        console.log("currAccNum", accountNumber);
        console.log("Bill Comp Model Fields End");*/ //TODO Replace while integration
        return '/api/account/' + accountNumber + '/site/' + selectedSiteID +
            '/billComparison';
    },
    defaults: {
        currentBillData: "",
...
var invoiceLineItemsModel = Backbone.Model.extend({
    ...
    url: function() {
        ...
        return '/api/account/' + accountNumber + '/invoiceLineItems';
    }
});
...
var PaymentPlanModel = Backbone.Collection.extend({
    ...
    url: function() {
        return '/api/account/' + accountNumber + '/paymentPlanData';
    },
});
...
var openPaymentCenter = function(accNum, amount) {
    var current = getCookie('ACN').split(',');
    for (var i = 0; i < current.length; i++) {
        if (current[i] == accNum) {
            createExpirableCookie('ACN', accNum, webSecurityTokenExpiry(), "/");
        }
    }
    var maskedAcc = utils.obscureNumber(accNum);
    window.open('/wps/wcm/myconnect/dte-web/quicklinks/pay-bill?' + maskedAcc +
        '=' + amount + '&showSelectedAccounts=true', '_self', false);
}
var openPaymentCenterSelectedBills = function() {
    window.open('/wps/wcm/myconnect/dte-web/quicklinks/pay-bill?' + selAccPay +
        '&showSelectedAccounts=true', '_self', false);
}
...
var currentBillEnroll = Backbone.Model.extend({
    url: "/api/bnpPrograms"
});

var currentEnroll = new currentBillEnroll();